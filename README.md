...
dataset is **Pikachu** (`49388`) — 13 suitability violations (7 Error, 6 Warning),
5 SAA deviations out of band, 1 concentration flag.
...
```bash
python3 seed_demo_state.py        # auto-discovers the best client and writes .state/<ref>.json
```

`seed_demo_state.py` automatically picks the individual client with the most
priorities and SAA deviations in the loaded database (no hardcoded ID needed).
You can override with `--client <ClientId>`.
...
| `briefing_generator.py` | Standalone model call used by scripts and tests: calls OpenAI Chat Completions, returns `{recent_development, health_check, outlook_and_actions, read_time_estimate_seconds}`. **Not called by `server.py`** — the API has its own `_generate()` with a different prompt and output schema; see `server.py` section below. |
...
```json
{
  ...
  "attention": [
    {"level": "critical|warning|info", "title": "… (must contain a number)", "detail": "… (must contain a number)", "source": "Human-readable label (e.g. 'Suitability violations')"}
  ],
...
call — never generated by the model. `attention[].source` is generated by the
model but constrained to a human-readable label (not an internal field name).

**What the model receives** — `_generate()` calls `context_to_prose()` and
assembles this user message (all fragments are pre-computed Python, no
inference by the model):

```
{client_and_portfolio}

RECENT PERFORMANCE:
{performance}

ISSUES AND RISKS:
{priorities}   ← violations + SAA breaches + concentration flags, with exact numbers

SAA TARGET DEVIATIONS (all dimensions out of band):
{saa_deviations}   ← always sent even when below the priority threshold, so clean
                     clients with large allocation drift are not silently blind-spotted
...
- Every `attention` title and detail must contain at least one number, % or CHF amount
- Maximum 4 attention items; related findings merged
...
A one-shot audit of the `/api/briefing` endpoint against 5 representative
clients: one with many rule violations (Pikachu, 49388), one with strong SAA
drift (Ralph Kramden, 181997), one with concentration risk and no violations
(Spock, 17091), one clean client (Vito Corleone, 78645), and one mid-tier
case (Travis Bickle, 125070).

| File | Contents |
|---|---|
| `output_<client_id>.json` | Raw JSON response from a cold API call for each client |
| `timing.json` | Cold and cached response time in ms per client |
| `check_briefing.py` | Automated checker: loads each output and tests C1–C11 (schema, digit presence, zero-change, vague words, since_last date anchors, talking-point numbers, source readability, word count, language consistency, latency) |

Run the checker against saved outputs at any time:

```bash
python audit/check_briefing.py
```

The outputs in `audit/` were captured before the prompt and quality-gate fixes
were applied (they are the pre-fix baseline). Re-running against fresh
responses after a server restart will show improved scores.
